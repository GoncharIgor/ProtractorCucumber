...
module.exports = function () {
    ...
    this.Then(/^I see that AddNewComputer page is opened$/, () =>
       // expect(browser.getCurrentUrl()).to.eventually.equal('http://computer-database.herokuapp.com/computers/new'));
    ...
    this.Then(/^I see that 'Company' select\-box is visible$/, () =>
        expect(addNewComputerPageObject.isElementVisible(addNewComputerPageObject.companySelectBox)).to.eventually.be.true);

    this.Then(/^I see that label for 'Company' select\-box is visible$/, () =>
        expect(addNewComputerPageObject.isElementVisible(addNewComputerPageObject.companySelectBoxLabel)).to.eventually.be.true);

    this.Then(/^I see that 'Create this computer' button is visible$/, () =>
        expect(addNewComputerPageObject.isElementVisible(addNewComputerPageObject.createThisComputerButton)).to.eventually.be.true);
    ...
    this.When(/^I fill all fields on AddNewComputer page with data: "([^"]*)", "([^"]*)", "([^"]*)", "([^"]*)"$/, (name, introducedDate, discontinuedDate, company) =>
